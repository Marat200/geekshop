...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        action_3_condition = Q(order__updated_at__lte=F('order__created_at') + action_1_delta)
        ...
        for item in orders_item_list:
            print(
                f'{item.action_order:2}: заказ №{item.pk}: {item.product.name:20}: '
                f'{item.discount_price:6.2f}: {item.updated_at - item.created_at}'
            )
